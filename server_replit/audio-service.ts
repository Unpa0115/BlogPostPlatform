...
export interface AudioProcessingResult {
  ...
  inputFile: string;
  timestamp: string;
  outputFiles?: string[]; // Support direct output files array
  output_files?: string[]; // Python snake_case version
  steps: {
    silenceRemoval?: {
      success: boolean;
      outputFile?: string;
  ...
  summary: {
    ...
    success: boolean;
    outputFiles?: string[];
    output_files?: string[]; // Python uses snake_case
  };
  error?: string;
}
...
export class AudioService {
  ...
  constructor() {
    ...
    this.scriptPath = path.resolve('./audio_processor.py');
  }
